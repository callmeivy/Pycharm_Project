...
def get_one_topic_doc():
    ...
    for one_type in channel:
        ...
        #         one_type.encode('utf-8')))
        sqlcursor.execute('''select pk,news_under_topic from top_topic_trend_week where left(created_date,13) = '2017-11-27 15';''' )
        # sqlcursor.execute('''select pk,news_under_topic from top_topic_trend_week where left(created_date,13) = '2017-11-27 15' limit 2;''' )
        summary_data = list(sqlcursor.fetchall())
        print 'topic number:',len(summary_data)
        topics_content = list()
        for topics in summary_data:
            topic_content = list()
            titles = topics[1].split(';')
            pk = topics[0]
            # TOPIC中选择2篇文章
            # titles = topics[1].split(';')[:3]
            # print len(titles)
            for doc in titles:
                if "'" in doc:
                    doc = doc.replace("'", "\\'")
            ...
            # topic_content_pk[topics[1]] = pk
        print "original doc reading is done!"
    # print len(topics_content)
    return topics_content

def key_word_priority():
    sqlConn = MySQLdb.connect(host='192.168.168.105', user='root', passwd='', db='cctv', charset='utf8')
    sqlcursor = sqlConn.cursor()
    completeComments = get_one_topic_doc()
    print "主题数：",len(completeComments)
    docs_key_words = []
    for completeComment in completeComments:
        # print type(completeComment)
        # print "jjjj",completeComment[0]
        ...
        for text in completeComment:
            first_one = list()
            result = AKE.score_keyphrases_by_textrank(text,n_keywords=70)
            # print type(result)
        ...
        print "len(completeComment)",len(completeComment)
        for i in range(0,len(completeComment)):
            ...
            sortedCandidates = sorted(candidate_score.iteritems(), key=operator.itemgetter(1), reverse=True)
            candidateList = []
        ...
        # print "docs_key_words",docs_key_words
        # c = collections.Counter(topic_key_words)
        # topic_words =  c.most_common(30)
        # for aa in topic_words:
    print len(docs_key_words)
    for i in range(0,len(docs_key_words)):
        key_words = " ".join(docs_key_words[i])
        print 'final key word:',key_words
        sqlcursor.execute("select ti from q_test where content = '%s' and channel = '%s' limit 1" % (completeComment[i], 'CCTV5'))
        title = list(sqlcursor.fetchone())[0]
        # print "title",title
        sqlcursor.execute('''update top_topic_trend_week set key_words = '%s' where news_under_topic like '%%%s%%';'''%(key_words,title))
        sqlConn.commit()
    sqlcursor.close()
