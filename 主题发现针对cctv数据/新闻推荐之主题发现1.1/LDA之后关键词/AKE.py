...
def score_keyphrases_by_textrank(text, n_keywords=50):
    ...
    keyphrases = dict()
    word_2 = TextRank4Keyword()
    word_2.analyze(text, window=2, lower=True)
    w_list = word_2.get_keywords(num=30, word_min_len=1)
    for w in w_list:
        if w.word not in stopwordList:
            if len(w.word)>1:
                # 两种分词方法的交集
                if w.word in word_nominal.keys():
                    if word_nominal[w.word] in valid:
                        # print 'ddddd',w.word,word_nominal[w.word]
