...
def corpus_to_list():
    ...
    valid = ['n','ns','nz','nr','nrt','nt','x']
    # valid = ['ns','nz','nr','nrt','nt','x']
    # channel = ["CCTV1","CCTV2"]
    # 全
    ...
    for one_type in channel:
        ...
        sqlcursor.execute('''SELECT count(*) from q_test where channel =  "%s" and length(content) >0 and ti NOT REGEXP '测试|test';''' %(one_type))
        total = list(sqlcursor.fetchone())[0]
        print "该频道下新闻：", total
        # base_number_id_index = int(total*0.9)
        # 像CCTV2,新闻特别多，就写0.1吧
        base_number_id_index = int(total*0.5)
        ...
        # sqlcursor.execute('''SELECT ti,content from q_test where channel =  "%s" and id < "%s" and id > "%s" and length(content) >0 and ti NOT REGEXP '测试|test';''' %(one_type, base_number_id_end_2,base_number_id_end))
            ...
            if PubTitle not in title_box:
                ...
                for i in full_text:
                    if i.word not in stopwordList:
                        if '某' not in i.word and '女士' not in i.word and '大姐' not in i.word:
                            if len(i.word) > 1:
        ...
        word_tfidf = dict()
        # listcount是大杂烩，所有文章的词全部堆在一起；all_docs_to_lists是一篇文章的词粘在一起，然后堆一块儿
        for i in listcount:
            word = i[0]
            timesss = i[1]
            ...
            for j in all_doc_into_list:
                if i[0] in j:
                    ind_2 += 1
            # print "word_idf",w.word,ind_2,ind
...
def excuteldamodel(mysqlhostIP, how_many_topics, how_many_iteration, how_many_topic_words,catcat,mysqlUserName = 'root', mysqlPassword = '', dbname = 'cctv'):
    ...
    mysqlcursor.execute('''CREATE TABLE IF NOT EXISTS top_topic_trend_summary(
...
# 以下是各个主题下新闻的列表
    ...
    thefile = open(('values_month %s.txt' %(catcat)), 'w+')
    ...
    for i, topic_dist in enumerate(topic_word):
        ...
        mysqlcursor.execute("insert into top_topic_trend_summary(category,topic_id, core_vector, key_words, news_under_topic, period, created_date) values (%s, %s, %s, %s, %s, %s, %s)", tempInsert)
        tempInsert = list()
        mysqlconn.commit()
    mysqlconn.close()
