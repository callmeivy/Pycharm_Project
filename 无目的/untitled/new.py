...
iae_hitlog_d=conn.gehua.iae_hitlog_d
inter_userinfo=conn.gehua.inter_userinfo
inter_userinfo.create_index("caid")
iae_hitlog_d.create_index("ip")
ip_ds=iae_hitlog_d.distinct("ip")
#ip=conn.gehua.ip
#res=ip.find()
ind=0
rowNumber=0
...
mysqlconn=MySQLdb.connect(host="10.3.3.182",user="root",passwd="",db="demo_vsp_d")
#for result in res:
for ip_d in ip_ds:
    ...
    if ip_d.startswith("172.16"):
        continue
    lines=iae_hitlog_d.find({'ip':ip_d}).sort('date')
...
#change the type list into the 14-dimension vector
                ...
                #string type can not be supported by those tools,need to change into the list type
                # programtype='0'*(21-len(str(sum)))+str(sum)
                # programtype=str(sum)
                # print "programtype",str(sum)
                # cur.close()
                try:
