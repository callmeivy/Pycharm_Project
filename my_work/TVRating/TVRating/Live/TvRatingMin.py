...
iae_audiencerate_new=conn.gehua.iae_audiencerate_20150515
...
# days比普通的-1
date = (datetime.datetime.now() - datetime.timedelta(days = 33))
...
mysqlcursor.execute('''CREATE TABLE IF NOT EXISTS audienceRate_min_0617(
...
for one_channel in channel:
    ...
    # print N
    lines=eval("iae_audiencerate_new.find({'WIC.date':'%s','WIC.A.n':'%s'}).batch_size(30)" %(date,str(one_channel)))
        ...
        for line in lines:
            ...
            if type(line['WIC']['A']) is list:
                for s in range(0,L):
                    ...
                    if line['WIC']['A'][s]['n']== one_channel:
                        # print 111111111
                        starttime=line['WIC']['A'][s]['s']
                        s_hour=int(starttime[0:2])
                        # print "sssssssssssss_hour",s_hour
    ...
    for element in a:
        ...
        mysqlcursor.executemany("insert into audienceRate_min_0617(date_time, minute_time, channel, tvRating) values (%s, %s, %s, %s)" , (insertList))
        mysqlconn.commit()
        count += 1
