...
def data_from_log():
    lines=eval("iae_audiencerate_new.find({'WIC.date':'%s','WIC.A.sn':'%s'}).batch_size(30)" %('2014-12-05','浙江卫视'))
...
def get_time(line,s):
    ...
    e_turnto_min=e_hour*60+e_min
    inter_min=e_turnto_min-s_turnto_min
    return s_turnto_min,e_turnto_min

def get_time_nolist(line):
    starttime=line['WIC']['A']['s']
    s_hour=int(starttime[0:2])
    s_min=int(starttime[3:5])
    s_sec=int(starttime[6:8])
