...
def corpus_to_list():
    ...
    for one_type in category:
        ...
        sqlcursor.execute("SELECT PubTitle,Storyline from cctv_news_content where tname like '%%%s%%' and (ScriptIssTime between '2016-10-01 00:00:00' and '2016-12-31 23:59:59')" %(one_type,))
        ...
        for row in tdm.rows(cutoff=1):
            ...
            if tokens_flag < 2:
                ...
                for word in row:
                    mark += 1
                    if mark != len(row):
                        ffile.write("%s\n" % str(word).encode('utf-8'))
                    else:
                        ffile.write("%s" % str(word).encode('utf-8'))
            else:
                mark2 +=1
                if mark2 != ind:
                    ori_matrix_file.write("%s\n" % str(row).encode('utf-8'))
                else:
                    ori_matrix_file.write("%s" % str(row).encode('utf-8'))


        ffile.close()
        ori_matrix_file.close()
        print 'reuters.tokens is ready.',len(row)
        print 'part-00000.txt is ready.',ind

            # print row
            # print len(row)
        # exit(0)
        # list_of_lists_file = open(root_directory + '/list_of_lists20170715.txt', 'w+')
        # list_of_lists_file.write("%s\n" % one.encode('utf-8'))
        # print len(allDoc_coma_join_lists)
        print 222222
        # 以下准备reuters.titles
        title_file = open(root_directory_lda+'/reuters.titles', 'w+')
        mark3 = 0
        ...
        excuteldamodel(mysqlhostIP='192.168.168.105', how_many_topics = 20, how_many_iteration=100, how_many_topic_words = 15,catcat = one_type,
                       dbname='cctv', )
