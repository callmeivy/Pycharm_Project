...
def corpus_to_list():
    ...
    for one_type in category:
        print one_type
        word_box = list()
        # sqlcursor.execute("SELECT count(*) from cctv_news_content where tname like '%%%s%%' and (ScriptIssTime between '2013-12-25 00:00:00' and '2013-12-30 23:59:59')" % (one_type,))
        sqlcursor.execute("SELECT PubTitle,Storyline from cctv_news_content where tname like '%%%s%%' and (ScriptIssTime between '2013-12-25 00:00:00' and '2013-12-30 23:59:59')" % (one_type,))
        ...
        # print "fetch how many",len(traindata)
        ind = 0
        ...
        excuteldamodel(mysqlhostIP='10.3.3.182', how_many_topics = 10, how_many_iteration=100, how_many_topic_words = 15,catcat = one_type,
                       dbname='cctv', )
...
def excuteldamodel(mysqlhostIP, how_many_topics, how_many_iteration, how_many_topic_words,catcat,mysqlUserName = 'root', mysqlPassword = '', dbname = 'cctv'):
    ...
    with open('E:/lda.model', 'rb') as f:
        model = cPickle.load(f)

    doc_topic = model.fit_transform(X)  # model.fit_transform(X) is also available
    # print "topic", topic
    ...
    # mysqlcursor.execute('''CREATE TABLE IF NOT EXISTS top_topic_trend_v2(
    #         pk bigint NOT NULL PRIMARY KEY AUTO_INCREMENT, category VARCHAR(50), topic_id VARCHAR(200), core_vector varchar(250), key_words VARCHAR(500), news_under_topic text, created_date datetime) charset=utf8''')
...
# 以下是各个主题下新闻的列表
    ...
    title_topic_low = defaultdict(list)
    ...
    for i in range(len(titles)):
        ...
        if isValid(i, 0.7):
            ...
            # title_topic_low是没有被归到任何topic底下的文章
            title_topic_low[doc_topic[i].argmax()].append(titles[i])
        print("{} (top topic: {})".format(doc_topic[i].argmax(),titles[i]))
    ...
    # max_title_index = doc_topic.argmax(axis=0)
    doc_topic_again = list(doc_topic)
    thefile = open('values_apply.txt', 'w+')
    for ii in doc_topic_again:
        thefile.write("%s\n" % str(ii).encode('utf-8'))
    thefile.close()
    # 将新来的新闻纳入原来的几个topic
    for k, v in title_topic.items():
        mysqlcursor.execute("update top_topic_trend_v2 set news_under_topic = concat(news_under_topic, %s) where topic_id = %s",(",".join(v),k))
        # mysqlcursor.commit()
    ...
    #     mysqlcursor.execute("insert into top_topic_trend_v2(category,topic_id, core_vector, key_words, news_under_topic, created_date) values (%s, %s, %s, %s, %s, %s)", tempInsert)
    #     tempInsert = list()
    #     mysqlconn.commit()
