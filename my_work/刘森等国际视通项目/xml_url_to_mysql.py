...
def xml_to_mysql(mysqlhostIP, mysqlUserName = 'root', mysqlPassword = '', dbname = 'weibo'):
    ...
    Source varchar(50), ScriptCreateTime varchar(20),ScriptIssTime DATETIME,lastmodifytime varchar(20),
    ...
    for item in root:
        ...
        for ref in item:
            ...
            if ref.attrib.values()[0] == 'ScriptIssTime':
                ScriptIssTime = ref.text
                tempData.append(ScriptIssTime)
        ...
        if len(tempData) == 7:
            sqlcursor.execute('''insert into cctv_news_content(MID,PubTitle,Summary,Storyline,ScriptIssTime, tname,updated_datetime) values (%s, %s, %s, %s, %s, %s, %s)''', tempData)
            sqlConn.commit()
...
if __name__ == '__main__':
    # cctv_news_content = xml_to_mysql(mysqlhostIP='10.3.3.182', dbname='cctv')
    cctv_news_content = xml_to_mysql(mysqlhostIP='192.168.168.105', dbname='cctv')