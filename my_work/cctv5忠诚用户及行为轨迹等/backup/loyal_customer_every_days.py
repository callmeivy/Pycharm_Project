...
def behavior(mysqlhostIP, mysqlUserName = 'root', mysqlPassword = '', dbname = 'weibo'):
    ...
    cur.execute("""SELECT distinct(common_device_id) FROM cctv5_app_behavior where (substring(page_page_start_time,1,10)) between %s and %s""", (start_date,end_date))
    # test
    # cur.execute("""SELECT distinct(common_device_id) FROM cctv5_app_behavior where (substring(page_page_start_time,1,10))=%s limit 5""", (i,))
    all_device = list(cur.fetchall())
...
# print all_device
    ...
    for key, valuesiter in groupby(sorted_input, key=getKey):
        ...
        result.append(dict(type=key, items= len(list(v[0] for v in valuesiter))))
