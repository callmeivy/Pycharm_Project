...
def corpus_to_list(baseurl,mysqlhostIP, topic_number, mysqlUserName = 'root', mysqlPassword = '', dbname = 'btv_v2'):
    ...
    sqlcursor.execute("SELECT DISTINCT(program_type_name) from ini_app_program_source_rel")
    # sqlcursor.execute("SELECT DISTINCT(program_type_name) from ini_app_program_source_rel where program_type_name = '军事' limit 1;")
    bufferTemp = sqlcursor.fetchall()
    for program_type_name in bufferTemp:
        program_type_name = program_type_name[0]
        print '新闻类别：', program_type_name
        word_box = list()
        # 表名
        ...
        # if program_type_name not in type_already_exist:
        #     type_already_exist.append(program_type_name)
        # else:
        #
        ind = 0
    ...
    # 以下为原来的formal_matrix_title.py***********************
        ...
        # return h_m_t
        excuteldamodel(baseurl = "http://172.28.12.34:8080", mysqlhostIP = '172.28.34.16', how_many_topics = h_m_t, topic_type = program_type_name, how_many_iteration = 100, how_many_topic_words = 8, dbname = 'btv_v2')
...
def excuteldamodel(baseurl,mysqlhostIP, how_many_topics, topic_type, how_many_iteration, how_many_topic_words, mysqlUserName = 'root', mysqlPassword = '', dbname = 'btv_v2'):
    ...
    mysqlcursor.execute('''CREATE TABLE IF NOT EXISTS topic_populaity(
            pk bigint NOT NULL PRIMARY KEY AUTO_INCREMENT, topic varchar(50), key_words VARCHAR(50), date DATE, popularity bigint(20), rank bigint(20), pic VARCHAR(250),\
             created_date datetime, topic_id VARCHAR(250), program_type_id varchar(50)) charset=utf8''')
    ...
    for i, topic_dist in enumerate(topic_word):
        ...
        # 文章的date
        i = str(topic_type) + str(i)
        print 'new topic number', i
        mysqlcursor.execute("select date from related_news where topic_id = %s limit 1", (i,))
        bufferTemp = mysqlcursor.fetchone()
        news_date = str(bufferTemp[0]).split(' ')[0]
        # print 'hah',news_date
        tempInsert.append(news_date)
        # 热度，基于click_number以及评论number
        mysqlcursor.execute("select sum(popularity) from related_news where topic_id = %s", (i,))
        bufferTemp = mysqlcursor.fetchone()
        ...
        tempInsert = list()
        mysqlconn.commit()
    print "看这里", topic_type
    mysqlcursor.execute ("insert into topic_populaity (topic, key_words, date, popularity, created_date, topic_id) select topic, key_words, date, popularity, created_date, topic_id from top_topic_trend where left(created_date,10) = %s and left(topic_id,2)= %s ",(today_date,topic_type))
    mysqlcursor.execute ("update topic_populaity set program_type_id =(select c.pti from (select distinct(a.program_type_id) as pti,b.program_type_name as ptn from competition_analysis as a inner join ini_app_program_source_rel as b on a.program = b.program_name) c where c.ptn=%s) where left(topic_id,2)= %s", (topic_type,topic_type))
    mysqlconn.close()
