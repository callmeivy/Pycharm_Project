...
# number是限定词云个数
def addhighFreqencyWords(baseurl,mysqlhostIP, programid, number,mysqlUserName = 'root', mysqlPassword = 'ctvit.123', dbname = 'btv_v2'):
# 读停用词
    ...
    sqlcursor.execute('''CREATE TABLE IF NOT EXISTS org_recognition(pk bigint NOT NULL PRIMARY KEY AUTO_INCREMENT, key_words varchar(200), frequency bigint(20), related_info varchar(200), date Date,
    ...
    # body_content = '我在乐视网看了《军情解码》20160509：莫斯科红场今日再阅兵 红场举行盛大阅兵仪式'
    #             Corpus_cut =  list(pseg.cut(body_content))
                Corpus_cut = Corpus_cut + list(pseg.cut(body_content))
# 将空格以及停用词去除
        org_box = list()
        Corpus_cut_new = list()
        for i in Corpus_cut:
            if (i.word !=' ') and len(i.word) > 1:
                if i.word not in stopwordList:
                    if i.flag =='nt':
                        # print i.word
                        org_box.append(i.word)
        Corpus_cut = Corpus_cut_new

    # 词频及排序
        kandian_listcount = dict(Counter(org_box))
        ...
        for item in kandian_listcount:
            ...
            sqlcursor.execute('''insert into org_recognition(key_words, frequency, rank, related_info, date, created_date, program_id, program) values (%s, %s, %s, %s, %s, %s, %s, %s)''', tempData)
            sqlConn.commit()
            tempData = []
    sqlConn.close()
...
if __name__=='__main__':
    commentTest = addhighFreqencyWords(baseurl = "http://172.28.12.34:8080", mysqlhostIP = '172.28.60.20', programid = '100', number = '20')
