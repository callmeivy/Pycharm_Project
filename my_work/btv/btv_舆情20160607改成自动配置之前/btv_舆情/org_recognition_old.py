...
def org_recognition(mysqlhostIP, mysqlUserName = 'root', mysqlPassword = '', dbname = 'btv_v2'):
    ...
    sqlcursor.execute('''INSERT INTO org_recognition(content, date, program_id, program) select content, date, program_id, program from customer_evaluation where date = %s;''', (otherStyleTime,))
    sqlConn.commit()
    sqlcursor.execute('''select content from org_recognition where date = %s;''', (otherStyleTime,))
