...
# 如果要跑昨天的代码，改这里%%%%%%%%%%%%%%%%%%%%%%%%%
mysqlcursor.execute('''DELETE from temp where date=DATE(DATE_SUB(NOW(), INTERVAL 5 DAY))''' )
# 如果要跑昨天的代码，改这里%%%%%%%%%%%%%%%%%%%%%%%%%
mysqlcursor.execute('''insert into temp (select cdn,ip,ngid,freq,LEFT(errorcode, 4), sum(count) as count,  datestyle from errorexport where errorcode like '%5225%' and datestyle =DATE(DATE_SUB(NOW(), INTERVAL 5 DAY)) group by cdn,ngid,freq,ip,LEFT(errorcode, 4))''' )
# 如果要跑昨天的代码，改这里%%%%%%%%%%%%%%%%%%%%%%%%%
mysqlcursor.execute('''insert into temp (select cdn,ip,ngid,freq,errorcode, sum(count) as count, datestyle from errorexport where errorcode not like '%5225%' and datestyle =DATE(DATE_SUB(NOW(), INTERVAL 5 DAY)) group by cdn,ngid,freq,ip,errorcode)''' )
...
# 如果要跑昨天的代码，改这里%%%%%%%%%%%%%%%%%%%%%%%%%
mysqlcursor.execute('''select distinct(datestyle)from errorexport where DATE_ADD(datestyle,INTERVAL "33" DAY)>now() order by datestyle desc''')
