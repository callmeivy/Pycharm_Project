...
# errorCodeList=['02-300','5225','02-300','02-300']
# 如果要跑昨天的代码，改这里%%%%%%%%%%%%%%%%%%%%%%%%%
inter=8
today = datetime.date.today()
# 如果要跑昨天的代码，改这里%%%%%%%%%%%%%%%%%%%%%%%%%
interday = datetime.timedelta(days=36)
day_29_before = today - interday
#rowNumber以及新建excel,打开旧的excel放到第一重循环内
for area in cdn_list:
    print area[4:5]

    oldWb = xlrd.open_workbook(r'/tmp/ErrorReportPro/errorReport/report/CDN_'+area[4:5]+'/GH-CDN-'+area[4:5]+'-NGid02300.xls',formatting_info=True)
    w=xlwt.Workbook(encoding = 'utf-8',style_compression=2)
    w = copy(oldWb)
    print "2"
    ...
    today = datetime.date.today()
    # 如果要跑昨天的代码，改这里%%%%%%%%%%%%%%%%%%%%%%%%%
    oneday = datetime.timedelta(days=8)
    ...
    mysqlcursor.execute('''select distinct(ngid) from temp where cdn=%s and errorcode = %s and date=DATE(DATE_SUB(NOW(), INTERVAL %s DAY))''',(area,'02-300',inter))
    ...
    for one_dis_ngid in dis_ngid:
        ...
        mysqlcursor.execute('''select DISTINCT count,freq,date from temp where cdn=%s and errorcode = %s and ngid=%s and date=DATE(DATE_SUB(NOW(), INTERVAL %s DAY)) ''',(area,'02-300',one_dis_ngid[0],inter))
...
####should have one tab space below, or there will be error of out of range
    ...
    w.save(r'/tmp/ErrorReportPro/errorReport/report/CDN_'+area[4:5]+'/GH-CDN-'+area[4:5]+'-NGid02300.xls')
            # w.save(r'E:\GH-CDN-'+area[4:5]+'-NGid02-300new.xls')
    ...
    if m==0:
        ...
        ws.panes_frozen= True
        ws.horz_split_pos= 1
        # w.save(r'/tmp/ErrorReportPro/errorReport/report/CDN_A/GH-CDN-A-NGid02-300.xls')
        append_index = len(w._Workbook__worksheets)-1
        w.set_active_sheet(append_index)
        w.save(r'/tmp/ErrorReportPro/errorReport/report/CDN_'+area[4:5]+'/GH-CDN-'+area[4:5]+'-NGid02300.xls')
