...
# cdn_list=['CDN_A']
# errorCodeList=['5206','5225','01-300','02-300']
# 如果要跑昨天的代码，改这里%%%%%%%%%%%%%%%%%%%%%%%%%
inter=8
...
for area in cdn_list:
    ...
    oldWb = xlrd.open_workbook(r'/tmp/ErrorReportPro/errorReport/report/CDN_'+area[4:5]+'/GH-CDN-'+area[4:5]+'-NGid5206.xls',formatting_info=True)
    w=xlwt.Workbook(encoding = 'utf-8',style_compression=2)
    w = copy(oldWb)
    print "2"
    ...
    mysqlcursor.execute('''select distinct(ngid) from temp where cdn=%s and errorcode = %s and date=DATE(DATE_SUB(NOW(), INTERVAL %s DAY))''',(area,'5206',inter))
    ...
    for one_dis_ngid in dis_ngid:
        ...
        mysqlcursor.execute('''select DISTINCT count,freq,date from temp where cdn=%s and errorcode = %s and ngid=%s and date=DATE(DATE_SUB(NOW(), INTERVAL %s DAY)) ''',(area,'5206',one_dis_ngid[0],inter))
...
####should have one tab space below, or there will be error of out of range
    ...
    w.save(r'/tmp/ErrorReportPro/errorReport/report/CDN_'+area[4:5]+'/GH-CDN-'+area[4:5]+'-NGid5206.xls')
            # w.save(r'E:\GH-CDN-'+area[4:5]+'-NGid5206new.xls')
