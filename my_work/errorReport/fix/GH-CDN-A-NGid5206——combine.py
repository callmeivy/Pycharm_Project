...
for one_cdn in cdn_list:
    area_single=one_cdn[4:5]
    for one_temp_date in dateList:
        if one_temp_date[0] is not None:
            ...
            mysqlcursor.execute('''select distinct(ngid) from temp where cdn=%s and errorcode like %s and substring(date,1,10)=%s''',(one_cdn,'5206%',one_temp_date[0]))
            ...
            for one_dis_ngid in dis_ngid:
                ...
                mysqlcursor.execute('''select DISTINCT count,freq from temp where cdn=%s and errorcode like %s and ngid=%s and substring(date,1,10)=%s ''',(one_cdn,'5206%',one_dis_ngid[0],one_temp_date[0]))
        ...
        ####should have one tab space below, or there will be error of out of range
            w.save(r'/tmp/ErrorReportPro/errorReport/report/CDN_'+area_single+'/GH-CDN-'+area_single+'-NGid5206.xls')
